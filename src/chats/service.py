...
class ChatsService:
    ...
    async def update_last_message(self, chat_id: PyObjectId, new_message: Message) -> None:
        await self.repository.update(chat_id, {"last_message": new_message.model_dump(), "message_count": new_message.id + 1})


chats_service = ChatsService(
    repository=ChatsRepository(collection=db["chats"])
)
