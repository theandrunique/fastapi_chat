...
async def get_existed_chat(chat_id: PyObjectId | UUID, payload: UserAuthorization) -> Chat:
    if isinstance(chat_id, PyObjectId):
        ...
        if not chat:
            chat = await chats_service.add(ChatFactory(type=ChatType.PRIVATE, members=[user_id, sender_id]))
